...
for rows in range(FirstRow, MaxRow + 1, 1):
    ...
    UserFinal = 'Hello ' + NameFix + ',\n\nYour device is listed among some of the highest Wireless costs to GE. Please review the breakdown of your' \
                ' billing below. To mitigate some of these costs when travelling, you should:\n\t1) Always book a travel package by e-mailing support@sydiangroup.com prior to your trip.\n\t2)Enable Airplane Mode, disable data usage, or exercise caution when using data. Roaming data is costly and often overused by background processes on mobile devices.\n\n---------------------------\n'
    ...
    MessageTop = MessageList[0:7]
    MessageRoaming = [row for row in MessageList if row.startswith(
        "Roaming") if 'Savings' not in row]
    MessageString = UserFinal + \
        '\n'.join(MessageTop) + '\n' + \
        MessageList[-3] + '\n\n' + '\n'.join(MessageRoaming)

    # adding an extra line break for readibility
    MessageString = MessageString.replace(
        'Roaming USA Data Charge', '\nRoaming USA Data Charge')
